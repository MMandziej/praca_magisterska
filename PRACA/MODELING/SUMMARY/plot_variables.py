...
plt.plot(nn_train_res.X_coordinates, nn_train_res.y_coordinates, color='#2f4b7c', lw=3, label = 'Zbiór treningowy')
plt.plot(nn_test_res.X_coordinates, nn_test_res.y_coordinates, color='#ff7c43', lw=3, label = 'Zbiór testowy')
plt.legend(loc = 'lower right')
...
# MODEL COMPARISON
# ROC
plt.plot(gbm_fpr_test, gbm_tpr_test, 'b', color='#ff7c43', lw=2, label = 'Gradient Boosting Machine (AUC = %0.3f)' % gbm_roc_auc_test)
plt.plot(crf_fpr_test, crf_tpr_test, 'b', color='#ffa600', lw=2, label = 'Las losowy (AUC = %0.3f)' % crf_roc_auc_test)
plt.plot(nn_fpr_test, nn_tpr_test, 'b', color='#444e86', lw=2, label = 'Siec neuronowa (AUC = %0.3f)' % nn_roc_auc_test)
plt.legend(loc = 'lower right')
plt.plot([0, 1], [0, 1], 'r--')
plt.xlim([0, 1])
plt.ylim([0, 1])
...
fig.add_subplot(ax)
# plt.title('Zdolnosc modelu do wykrywania upadlych firm - Sieć neuronowa')
plt.plot(gbm_test_res.X_coordinates, gbm_test_res.y_coordinates, color='#ff7c43', lw=2, label = 'Gradient Boosting Machine')
plt.plot(crf_test_res.X_coordinates, crf_test_res.y_coordinates, color='#ffa600', lw=2, label = 'Las losowy')
plt.plot(nn_test_res.X_coordinates, nn_test_res.y_coordinates, color='#444e86', lw=2, label = 'Siec neuronowa')     
plt.legend(loc = 'lower right')
ax.xaxis.set_major_formatter(mtick.PercentFormatter(1.0))
ax.yaxis.set_major_formatter(mtick.PercentFormatter(1.0))
# plt.plot([0, 1], [0, 1],'r--')
