function(dataset, time_feature = 'datestamp', feature, Label = 'all', time_frame = 'month', prop = T) {
  ...
  if (Label == 'all') {
    if (is.numeric(dataset[,feature])) {
      ...
      test_plot = test_plot %>%
        layout(yaxis = list(title = "% of cases for each group"),
    ...
    if(is.numeric(dataset[,feature])) {
      dataset[,time_feature] = floor_date(dataset[,time_feature], unit = time_frame)
      dataset[,feature] = quantcut(dataset[,feature], q = 5)
      test_1 = table(dataset[which(dataset[,'Label'] == Label), time_feature],
                     dataset[which(dataset[,'Label'] == Label), feature])
      if (prop == T) {
        test_1 = prop.table(test_1, margin = 1)
      }
      test_1 = as.data.frame(test_1)
