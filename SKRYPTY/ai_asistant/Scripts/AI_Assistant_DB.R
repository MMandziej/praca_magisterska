#your_dir='C:/Users/kzielinski003/Desktop/ai_asistant/'
...
dataset = read.csv("C:/Users/mmandziej001/Desktop/FCU/SCRIPTS/predictive_qc_db/data/processed/cib_pc_20200428.txt", sep = '|') #, stringsAsFactors=FALSE)
#dataset = read.csv("C:/Users/mmandziej001/Desktop/FCU/SCRIPTS/predictive_qc_db/data/processed/ai_assistant.txt", sep = '|')
dataset$Label = ifelse(dataset$Score==100,0,1)
dataset = as.data.frame(dataset)
dataset = dataset[1:1810,]
...
# cols_drop <- c('stage', 'Analyst' , 'QC' , 'checklist_submission')
# dataset[ , !(names(dataset) %in% drops)]
# replace SQL NULL values with NA for analyst and QC performance and outreach features
# dataset <- replace_with_na(data = dataset,
#                            replace = list(score_deducted_1 = 'NULL')



# change features type to numeric
# dataset <- transform(dataset,
#                      # score_deducted
#                      score_deducted_1 = as.numeric(levels(score_deducted_1))[score_deducted_1])


AI_Assistant=function(dataset,Variable_importance_all_methods=NA,colnames_feature_selection=NA,stage,nn_results=NA,gbm_results=NA,rf_results=NA){
  if(stage=="Exploratory_analysis"){
      ...
      
    {
      ...
      output$detailed_table=renderDataTable({
        ...
        detailed_table$`No. cases`=round(fraction*nrow(gbm_results[[1]]))
        detailed_table$`Errors found`=sum(gbm_results[[1]][round(fraction*nrow(gbm_results[[1]])),]$Sum)
        detailed_table$`Errors not found`=sum(gbm_results[[1]]$Label)-detailed_table$`Errors found`
