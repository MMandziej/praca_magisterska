  ...
  if(stage=="Exploratory_analysis"){
                                    ...
                                    tabPanel("Feature Selection", 
                                             box(width = 4,
                                                 ...
                                                 selectInput("Category_selection_fs", "Select category",
                                                             choices = nn_results1[[1]][['LabelType']],
                                                             selected = "Identyfing and Understanding the Client"
                                                 )
                                             ),
                                             box(width = 4,
                      ...
                      tabBox(
                        ...
                        tabPanel("Gradient Boosted Machine",
                                 ...
                                 selectInput("Category_selection_gbm", "Select labels",
                                             choices = gbm_results1[[1]][['LabelType']],
                                             selected = "Identyfing and Understanding the Client"
                                 ),
                                 box(width = 2,
                      ...
                      tabBox(
                        ...
                        tabPanel("Classification Random Forest",
                                 ...
                                 selectInput("Category_selection_rf", "Select labels",
                                             choices = rf_results1[[1]][['LabelType']],
                                             selected = "Identyfing and Understanding the Client"
                                 ),
                                 box(width = 2,
      ...
      
    {
      ...
      feature_selection_data = reactive({
        lasso <- filter(Variable_importance_all_methods[[1]], LabelType == input$Category_selection_fs)
        lasso <- lasso[,-which(colnames(lasso) %in% c('X1', 'LabelType'))]
        row.names(lasso) <- lasso$V2
        
        boruta <- filter(Variable_importance_all_methods[[2]], LabelType == input$Category_selection_fs)
        boruta <- boruta[,-which(colnames(boruta) %in% c('X1', 'LabelType', 'check'))]
        row.names(boruta) <- boruta$V2
        
        random_forest <- filter(Variable_importance_all_methods[[3]], LabelType == input$Category_selection_fs)
        random_forest <- random_forest[,-which(colnames(random_forest) %in% c('X1', 'LabelType'))]
      ...
      nn_results_data = reactive({
        dataset1_nn <- filter(nn_results[[1]], LabelType == input$Category_selection_nn)
        dataset1_nn <- dataset1_nn[,-which(colnames(dataset1_nn) %in% c('LabelType'))]
        dataset2_nn <- filter(nn_results[[2]], LabelType == input$Category_selection_nn)
        dataset2_nn <- dataset2_nn[,-which(colnames(dataset2_nn) %in% c('LabelType'))]
      ...
      gbm_results_data = reactive({
        dataset1_gbm <- filter(gbm_results1[[1]], LabelType == input$Category_selection_gbm)
        dataset1_gbm <- dataset1_gbm[,-which(colnames(dataset1_gbm) %in% c('LabelType'))]
        dataset2_gbm <- filter(gbm_results1[[2]], LabelType == input$Category_selection_gbm)
        dataset2_gbm <- dataset2_gbm[,-which(colnames(dataset2_gbm) %in% c('LabelType'))]
        gbm_results_filtered <- list(dataset1_gbm, dataset2_gbm)
      })
      
      rf_results_data = reactive({
        dataset1_rf <- filter(rf_results1[[1]], LabelType == input$Category_selection_rf)
        dataset1_rf <- dataset1_rf[,-which(colnames(dataset1_rf) %in% c('LabelType'))]
        dataset2_rf <- filter(rf_results1[[2]], LabelType == input$Category_selection_rf)
        dataset2_rf <- dataset2_rf[,-which(colnames(dataset2_rf) %in% c('LabelType'))]
      ...
      summary_results_data = reactive({
        dataset1_sm <- filter(nn_results1[[1]], LabelType == input$Category_selection_summary)
        dataset1_sm <- dataset1_sm[,-which(colnames(dataset1_sm) %in% c('LabelType'))]
        dataset2_sm <- filter(nn_results1[[2]], LabelType == input$Category_selection_summary)
        dataset2_sm <- dataset2_sm[,-which(colnames(dataset2_sm) %in% c('LabelType'))]
      ...
      gbm_results_table = reactive({
        dataset1_gbm <- filter(gbm_results1[[1]], LabelType == input$Category_selection_summary)
        dataset1_gbm <- dataset1_gbm[,-which(colnames(dataset1_gbm) %in% c('LabelType'))]
        dataset2_gbm <- filter(gbm_results1[[2]], LabelType == input$Category_selection_summary)
        dataset2_gbm <- dataset2_gbm[,-which(colnames(dataset2_gbm) %in% c('LabelType'))]
        gbm_results_filtered <- list(dataset1_gbm, dataset2_gbm)
      })
        nn_results_table = reactive({
        dataset1_nn <- filter(nn_results[[1]], LabelType == input$Category_selection_summary)
        dataset1_nn <- dataset1_nn[,-which(colnames(dataset1_nn) %in% c('LabelType'))]
        dataset2_nn <- filter(nn_results[[2]], LabelType == input$Category_selection_summary)
        dataset2_nn <- dataset2_nn[,-which(colnames(dataset2_nn) %in% c('LabelType'))]
        nn_results_filtered <- list(dataset1_nn, dataset2_nn)
      })
      
      rf_results_table = reactive({
        dataset1_rf <- filter(rf_results1[[1]], LabelType == input$Category_selection_summary)
        dataset1_rf <- dataset1_rf[,-which(colnames(dataset1_rf) %in% c('LabelType'))]
        dataset2_rf <- filter(rf_results1[[2]], LabelType == input$Category_selection_summary)
        dataset2_rf <- dataset2_rf[,-which(colnames(dataset2_rf) %in% c('LabelType'))]
        rf_results_filtered <- list(dataset1_rf, dataset2_rf)
      })
