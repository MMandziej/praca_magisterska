...
from model_training.scripts.NN_modelling_restructured import full_search, narrow_search, backward_search, forward_search


train_data = pd.read_csv('model_training/data/PC/9_Control/dataset_dummy_grouped_time_train_cat9.csv')
test_data = pd.read_csv('model_training/data/PC/9_Control/dataset_dummy_grouped_time_test_cat9.csv')
...
boruta_importance = pd.read_excel('model_training/data/ai_assistant_dumps/PC/9_Control/Boruta_variable_importance_pc.xlsx')
...
boruta_train_data = train_data#[boruta_features]
boruta_test_data = test_data#[boruta_features]


models_list, models_params, output_table = full_search(
    train_data, train_labels, test_data, train_labels, test_labels,
    max_iter=20000, max_hours=0.1,
    min_auc=0.3, max_overfit_auc=0.02,
    prefix='nn_results_all_features')

boruta_models_list, boruta_models_params, boruta_output_table = full_search(
    boruta_train_data, train_labels, boruta_test_data, train_labels, test_labels,
    max_iter=3, max_hours=17,
    min_auc=0.3, max_overfit_auc=0.02,
