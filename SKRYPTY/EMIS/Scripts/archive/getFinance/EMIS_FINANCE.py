...
JSON_OUTPUT_PATH = r'C:\Users\mmandziej001\Desktop\Projects\EMIS\OUTPUT_API\JSONS_FINANCE\\'
XLXS_OUTPUT_PATH = r'C:\Users\mmandziej001\Desktop\Projects\EMIS\OUTPUT_API\\'

######### INSERT FISCAL YEARS  ########
fiscal_years = ['2015', '2016', '2017','2018']
...
def get_financial_data(nips_isics: dict, 
    ...
    for nip, isic in nips_isics.items():
        # Create empty list for each company to append with missing fiscal years
        nips_years_list = []
        for fiscal_year in fiscal_years:
            ...
            try:
                ...
                # HTTPError is raised when for company there is no data for requested
                # fiscal year
                ...
                # Appenda with missing fiscal year if there is no data in EMIS                              
                nips_years_list.append(fiscal_year)
            ...
            # Append output dict with company with missing fiscal years
            nips_errors[nip] = 'No data for years: ' + ', '.join(nips_years_list)
...
def parse_financial_data(nips_jsons: list, nips_errors: dict) -> pd.DataFrame:
    ...
    full_output = full_output[financial_data_cols].sort_values(by=['nip', 'FiscalYear'], na_position='last').reset_index(drop=True)
    ...
    try:
        ...
        if not full_output.empty:
            ...
            for fiscal_year in fiscal_years_df:
                ...
                with pd.ExcelWriter(XLXS_OUTPUT_PATH + 'EMIS_FINANCE1_' + str(d.date.today()) + '.xlsx') as writer:
...
nip_list, nips_invalid, log_df = get_nip_name(INPUT_FILE)
