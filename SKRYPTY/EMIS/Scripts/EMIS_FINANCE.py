...
from emis_full import get_session_id, get_nip_name, get_isic_list, request_data,\
                 financial_data_cols_unprocessed, cols_rename, financial_data_cols, accounts
from ml_help import calculate_fait_tests
...
def get_financial_data(nips_isics: dict, 
    ...
    print("Data successfully downloaded. %s files collected and exported to %s" %(len(nips_jsons),
                                                                                FIN_JSON_OUTPUT_PATH[:-2]))
...
def export_output(full_output: pd.DataFrame,
    ...
    try:
        ...
        if not full_output.empty:
            ...
            print(fiscal_years_df, type(fiscal_years_df))
            ...
            for fiscal_year in fiscal_years_df:
                ...
                with pd.ExcelWriter(FIN_XLSX_OUTPUT_PATH + 'EMIS_FINANCE_' + str(d.date.today()) + '.xlsx') as writer:
                    ...
                    #fait_tests.to_excel(writer, 'FAIT_tests', index=False)
            ...
            output_list.extend([fait_tests, full_tests]) #, emis_input
    except Exception as e:
...
nips_errors = {}
nips_jsons_fin = []
path_to_json = r'C:\Users\mmandziej001\Desktop\Projects\FAIT\Prediction Module\POLAND_DANE\FINANCE\Operational\ALL\\'
...
for index, js in enumerate(json_files):
    with open(os.path.join(path_to_json, js)) as json_file:
        print(index)
        try:
            nips_jsons_fin.append({js[:10]: json.load(json_file)})
        except:
...
fiscal_years = ['2020']
nips = pd.read_excel(r"C:\Users\mmandziej001\Desktop\Projects\FAIT\Prediction Module\POLAND_DANE\Operational_downloaded_all.xlsx",
                     converters={'NIP': str, 'REGON': str, 'KRS': str})
nips = nips[(~pd.isnull(nips['NIP'])) &
            (np.isin(nips['NIP'], nipy, invert=True))]


#nips = nips[nips['FIN_LAST_YEAR'] == 2015]
#nips = nips[(nips['FIN_LAST_YEAR'] != 'brakdanych') &
#            (~pd.isnull(nips['NIP'])) &
#            (~pd.isnull(nips['FIN_LAST_YEAR']))] &
#            (nips['FIN_LAST_YEAR'] == 2018)]
nips_errors = {}
nips_isics = dict(zip([str(i) for i in list(nips['NIP'][:4000])],
                      [str(i) for i in list(nips['ISIC'][:4000])]))
"""
############### GET OUTPUT ##################
nip_dic, nips_invalid, log_df = get_nip_name(INPUT_FILE, ID_CLASS)
...
output_exported = export_output(output_parsed, output_fait_tests, output_all_tests)
